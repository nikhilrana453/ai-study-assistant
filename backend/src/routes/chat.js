...
router.post('/message/stream', authenticateToken, checkEnrollment, async (req, res) => {
  ...
  const systemPrompt = hintMode
    ? `You are a study tutor for "${course.name}".
       Use ONLY the TEXT below to give a short hint.
       Do not give direct answers — guide the student to think.
       Do not repeat yourself.
       Do not use markdown symbols like ** or * or #.
       Do not ask follow up questions.
       Write in plain sentences only.
       If the topic is not found in the TEXT below say only this exact sentence:
    ...
    : `You are a study tutor for "${course.name}".
       ...
       Write your answer ONCE — never repeat any point.
       Do not use markdown symbols like ** or * or #.
